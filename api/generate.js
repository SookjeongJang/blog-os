...
현재 요청에 실제 내부 연결글 URL이 제공되지 않았다면
관련글 버튼 자체를 만들지 않는다.

대신 글 마지막의 "다음에 읽으면 좋은 글"에는
제목만 텍스트로 추천할 수 있다.
...
<div class="series-next">
  ...
  <ul>
    <li>주택담보대출 한도에 영향을 주는 기준</li>
    <li>신용대출 심사에서 확인하는 항목</li>
  </ul>
</div>

실제 URL이 제공된 경우에만 아래처럼 링크 버튼을 만든다.
...
- 본문 문장 안에는 원시 URL을 직접 출력하지 않는다.
- Markdown 링크 형식 [텍스트](URL)을 사용하지 않는다.
- (사이트명)(URL) 같은 형태로 출처를 표시하지 않는다.
- 본문에서는 "국세청 안내에 따르면", "금융위원회 발표에 따르면"처럼 기관명만 자연스럽게 언급한다.
- 클릭 가능한 공식 URL은 글 마지막의 .official-source 영역에만 넣는다.
- URL에 utm_source=openai 등 불필요한 추적 파라미터를 붙이지 않는다.
- 제공받은 source_url 외의 URL을 임의로 만들지 않는다.
